...
<?php get_template_part('header-video-cat'); ?>
		<!-- content -->	
		<div class="content">
			...
			<div class="ff_panel-conainer fl-wrap">
				...
				<div class="gallery-items big-padding   four-column fl-wrap vis-thumb-info lightgallery">
					<?php global $loop; 
						$args = array_merge( $wp_query->query, array( 'post_type' => 'video', 'posts_per_page'=>-1, ) );
						query_posts( $args );
					?>	
					<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
				
						<?php $kotlis_portfolio_category = wp_get_post_terms($post->ID,'video_category');?>
						<?php 
							$kotlis_class = ""; 
							$kotlis_categories = ""; 
							...
							<?php if(get_post_meta($post->ID,'rnr_video_post_vid_url',true)):?>
							<div class="gallery-item <?php echo esc_attr($kotlis_class);?>">
								<div class="grid-item-holder hov_zoom">
                                    <?php if ($kotlis_options['video_hover_st'] == 'st2') {?>
									...
									<?php } ?>								
                                    <?php if ($kotlis_options['video_hover_st'] != 'st2') {?>
									     <a href="<?php echo esc_url(get_post_meta($post->ID,'rnr_video_post_vid_url',true));?>" class="box-media-zoom   popup-image"><i class="fal fa-play"></i></a>         
									<?php } ?>	
